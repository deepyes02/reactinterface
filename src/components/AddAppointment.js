...
const AddAppointment = () => {
	...
	return (
		<div>
			<button className={`bg-indigo-100 flex items-center text-black text-3xl content-center px-2 py-3 w-full text-left ${toggle ? 'rounded-t-md' : 'rounded-t'}`} onClick={() => setToggle(!toggle)}>
				<BiCalendarPlus /><h2>{toggle === true ? 'Hide ' : 'Add '}Appointment</h2>
			</button>
			{toggle === true ?
