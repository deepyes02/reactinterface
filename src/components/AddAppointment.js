...
const AddAppointment = () => {
	...
	const handleToggle = () => {
		...
		if (toggle) {
			document.getElementById('toggleinput').classList.remove('hidden');
			return setToggle(false)
		} else {
			document.getElementById('toggleinput').classList.add('hidden');
	...
	return (
		<div>
			<button className="bg-blue-400 text-white px-2 py-3 w-full text-left rounded-t-md" onClick={handleToggle}>
				<div><BiCalendarPlus className="inline-block align-text-top" />  Add Appointment</div>
			</button>
			{
				<div id="toggleinput" className="border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md pl-4 pr-4 pb-4">
